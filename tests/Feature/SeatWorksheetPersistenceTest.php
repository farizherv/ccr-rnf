...
class SeatWorksheetPersistenceTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_parts_styles_notes_and_tools_are_persisted_via_autosave(): void
    {
        ...
        $response = $this->actingAs($user)->postJson(
            route('seat.worksheet.autosave', ['id' => $report->id]),
            [
    ...
    public function test_update_header_accepts_payload_without_ts_when_client_revision_is_current(): void
    {
        ...
        $response = $this->actingAs($user)->put(
            route('seat.update.header', ['id' => $report->id]),
            [
        ...
        $response
            ->assertRedirect(route('ccr.manage.seat'))
    ...
    public function test_update_header_without_ts_treats_submit_as_authoritative_even_when_client_revision_is_old(): void
    {
        ...
        $response = $this->actingAs($user)->put(
            ...
            [
                'group_folder' => 'Operator Seat',
                'component' => 'Seat With Stale Submit',
                'inspection_date' => now()->toDateString(),
    ...
    public function test_store_persists_parts_styles_notes_tools_payload(): void
    {
        ...
        $response = $this->actingAs($user)->post(route('seat.store'), [
            'group_folder' => 'Operator Seat',
            'component' => 'Seat Stored',
            'inspection_date' => now()->toDateString(),
    ...
    public function test_store_uses_server_draft_payload_when_form_payload_is_incomplete(): void
    {
        ...
        CcrDraft::query()->create([
            ...
            'user_id' => (int) $user->id,
            'type' => 'seat',
            'client_key' => 'seat:test:draft',
            'draft_name' => 'Seat Draft Test',
            'parts_payload' => [
                'meta' => [
                    'template_key' => 'seat_blank',
                    'template_version' => 'v1',
        ...
        $response = $this->actingAs($user)->post(route('seat.store'), [
            'group_folder' => 'Operator Seat',
            'component' => 'Seat Draft Fallback',
            'inspection_date' => now()->toDateString(),
            'draft_id' => $draftId,
            'draft_client_key' => 'seat:test:draft',
    ...
    public function test_store_rejects_invalid_parts_payload_json(): void
    {
        ...
        $response = $this->actingAs($user)
            ->from(route('seat.create'))
            ->post(route('seat.store'), [
                'group_folder' => 'Operator Seat',
                'component' => 'Seat Invalid JSON',
                'inspection_date' => now()->toDateString(),
                'parts_payload' => '{"meta":{"template_key":"seat_blank"}', // broken JSON
                'detail_payload' => json_encode([
                    'meta' => ['template_key' => 'seat_blank', 'template_version' => 'v1'],
        ...
        $response
            ->assertRedirect(route('seat.create'))
    ...
    public function test_store_merges_sparse_incoming_formatting_with_richer_server_draft(): void
    {
        ...
        CcrDraft::query()->create([
            ...
            'user_id' => (int) $user->id,
            'type' => 'seat',
            'client_key' => 'seat:test:merge',
            'draft_name' => 'Seat Draft Merge',
            'parts_payload' => [
                'meta' => [
                    'template_key' => 'seat_blank',
                    'template_version' => 'v1',
        ...
        $response = $this->actingAs($user)->post(route('seat.store'), [
            'group_folder' => 'Operator Seat',
            'component' => 'Seat Merge Draft',
            'inspection_date' => now()->toDateString(),
            'draft_id' => $draftId,
            'draft_client_key' => 'seat:test:merge',
    ...
    public function test_autosave_rejects_oversized_seat_items_payload(): void
    {
        $user = User::factory()->create(['role' => 'admin']);
        $report = $this->makeSeatReport();
        ...
        $oversized = [
            [
                'uid' => 'si_1',
                'item' => str_repeat('X', (8 * 1024 * 1024) + 128),
            ],
        ];
        ...
        $response = $this->actingAs($user)->postJson(
            route('seat.worksheet.autosave', ['id' => $report->id]),
            [
                'seat_items_payload' => $oversized,
            ]
        );

        $response
            ->assertStatus(422)
            ->assertJsonValidationErrors(['seat_items_payload']);
    }
