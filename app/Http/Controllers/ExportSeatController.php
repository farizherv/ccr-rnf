...
class ExportSeatController extends Controller
{
    ...
    private function findSeatReportOrFail($id): CcrReport
    {
    ...
    {
        return $this->envInt('SEAT_PREVIEW_TIMEOUT_SECONDS', self::PREVIEW_CONVERT_TIMEOUT_SECONDS, 20, 600);
    }

    private function previewMaxDocxBytes(): int
    {
        return $this->envInt('SEAT_PREVIEW_MAX_DOCX_BYTES', self::PREVIEW_MAX_DOCX_BYTES, 1048576);
    }

    private function previewMaxPdfBytes(): int
    {
        return $this->envInt('SEAT_PREVIEW_MAX_PDF_BYTES', self::PREVIEW_MAX_PDF_BYTES, 1048576);
    }

    private function previewFailCooldownSeconds(): int
    {
        return $this->envInt('SEAT_PREVIEW_FAIL_COOLDOWN_SECONDS', self::PREVIEW_FAIL_COOLDOWN_SECONDS, 5, 300);
    }

    private function previewTmpSweepSeconds(): int
    {
        return $this->envInt('SEAT_PREVIEW_TMP_SWEEP_SECONDS', self::PREVIEW_TMP_SWEEP_SECONDS, 600, 604800);
    }
    ...
    {
        $this->jobBroker()->enqueue('preview', 'seat', $reportId);
    }

    private function queueWordBuild(int $reportId): void
    {
        $this->jobBroker()->enqueue('word', 'seat', $reportId);
    }
    ...
    private function tryInlinePreviewFallback(CcrReport $report): ?string
    {
        ...
        try {
            ...
            $this->jobBroker()->markSuccess('preview', 'seat', (int) $report->id);
            return $pdfAbs;
        } catch (Throwable $e) {
            Log::warning('SEAT preview fallback inline gagal', [
                'report_id' => $report->id,
                'error' => $e->getMessage(),
            ]);

            $this->jobBroker()->markFailure('preview', 'seat', (int) $report->id, $e->getMessage());
    ...
    private function ensureFreshDocx(CcrReport $report): string
    {
        ...
        return $this->withReportLock($reportId, function () use ($reportId) {
            $fresh = $this->findSeatReportOrFail($reportId);

            if ($this->shouldRegenerate($fresh)) {
                $abs = $this->generateSeat($reportId);
                $fresh->refresh();
                return $abs;
            }
    ...
    private function maybeSweepOldPreviewTempDirs(): void
    {
        ...
        foreach (glob($baseDir . '/ccr-preview-seat-*', GLOB_NOSORT) ?: [] as $path) {
            if (!is_dir($path)) {
                continue;
            }
    ...
    {
        return storage_path('app/tmp/export-locks/seat-preview-' . $reportId . '.failed.json');
    }
    ...
    private function ensurePreviewPdf(CcrReport $report): string
    {
        ...
        return $this->withPreviewLock($reportId, function () use ($reportId) {
            ...
            $tmpDir = storage_path('app/tmp/ccr-preview-seat-' . $reportId . '-' . Str::random(8));
            ...
            try {
                ...
                if (is_file($pdfAbs) && (@filesize($pdfAbs) ?: 0) > 0) {
                    Log::warning('SEAT preview memakai PDF lama karena regenerate gagal', [
                        'report_id' => $reportId,
                        'error' => $e->getMessage(),
                    ]);
    ...
    {
        $report = $this->findSeatReportOrFail($id);
    ...
    public function generateSeat($id): string
    {
        ...
        $report = $this->findSeatReportOrFail($id)->load('items.photos');
    ...
    public function preview($id)
    {
        ...
        if ($this->jobBroker()->queueEnabled()) {
            if ($this->previewReadyPath($report) === null) {
                if ($this->tryInlinePreviewFallback($report) === null) {
                    ...
                    $retryAfter = $this->jobBroker()->retryAfterSeconds('preview', 'seat', (int) $report->id);
            ...
            try {
                ...
                Log::warning('SEAT preview PDF gagal dibangun', [
                    'report_id' => $report->id,
                    'error' => $e->getMessage(),
                ]);
        ...
        return view('ccr.word_preview', [
            'report' => $report,
            'title' => 'PREVIEW CCR - SEAT',
            'subtitle' => 'Preview ini berasal dari file Word yang sama dengan file download.',
            'backUrl' => route('ccr.manage.seat'),
            'pdfPreviewUrl' => route('seat.preview.pdf', $report->id),
            'downloadWordUrl' => route('seat.export.word', $report->id),
    ...
    public function previewPdfFile($id)
    {
        ...
        if ($this->jobBroker()->queueEnabled()) {
            ...
            if ($pdfAbs === null) {
                ...
                if ($pdfAbs === null) {
                    ...
                    return response('Preview PDF sedang diproses di antrian. Silakan coba lagi sebentar.', 503, [
                        ...
                        'Pragma' => 'no-cache',
                        'Expires' => '0',
                        'Retry-After' => (string) $this->jobBroker()->retryAfterSeconds('preview', 'seat', (int) $report->id),
                    ]);
                }
            }
        } else {
            try {
                ...
                Log::warning('SEAT preview file gagal diakses', [
                    'report_id' => $report->id,
                    'error' => $e->getMessage(),
                ]);
    ...
    public function generateSeatDownload($id)
    {
        ...
        if ($this->jobBroker()->queueEnabled()) {
            if ($this->shouldRegenerate($report) || !$report->docx_path || !Storage::disk('public')->exists($report->docx_path)) {
                ...
                try {
                    ...
                    $this->jobBroker()->markSuccess('word', 'seat', (int) $report->id);
                } catch (Throwable $e) {
                    Log::warning('SEAT word download fallback inline gagal', [
                        'report_id' => (int) $report->id,
                        'error' => $e->getMessage(),
                    ]);
                    $this->jobBroker()->markFailure('word', 'seat', (int) $report->id, $e->getMessage());
                    ...
                    if (!$report->docx_path || !Storage::disk('public')->exists($report->docx_path)) {
                        return response('File Word sedang diproses di antrian. Silakan coba lagi sebentar.', 503, [
                            ...
                            'Pragma' => 'no-cache',
                            'Expires' => '0',
                            'Retry-After' => (string) $this->jobBroker()->retryAfterSeconds('word', 'seat', (int) $report->id),
                        ]);
                    }
        ...
        $downloadName = 'CCR_SEAT' . ($componentName !== '' ? '-' . $componentName : '') . '.docx';
    ...
    {
        return $this->withNamedLock('seat-word-' . $reportId, $callback);
    }

    private function withPreviewLock(int $reportId, callable $callback)
    {
        return $this->withNamedLock('seat-preview-' . $reportId, $callback);
    }
