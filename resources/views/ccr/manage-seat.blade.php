...
<div data-page="manage-seat">
    ...
    @php
        ...
        $seatPhotoDraftKey = $seatCcrDraftKey;
    ...
    <div class="box filter-box">
        <h3 class="section-title">Daftar CCR Seat</h3>
        ...
        <div class="filter-row">
            ...
            {{-- FILTER STATUS --}}
            <div class="filter-group filter-small">
                <label for="statusFilterSeat">Status</label>
                <select id="statusFilterSeat" class="input">
    ...
    >
        ...
        {{-- LIST --}}
        <div class="box report-list-box" id="reportListSeat" x-ref="list">
            <div class="list-head">
                <div class="list-head-title">List Laporan</div>
                <div class="list-head-tools">
                    <a href="{{ $seatCreateUrl }}"
                       class="btn-list-create js-draft-entry"
                       data-draft-target="seat"
                       data-target-url="{{ $seatCreateUrl }}">
                        + Buat CCR Seat
                    </a>
                    <div class="list-head-count">
                        <span id="resultCountSeat" class="count-number">{{ $pageVisibleReports }}</span>
            ...
            @forelse($reports as $r)
                ...
                <div class="report-card"
                    ...
                    <div class="report-actions">
                        <a href="{{ route('seat.edit', $r->id) }}" class="btn-premium edit-btn">Edit</a>

                        <a href="{{ route('seat.export.word', $r->id) }}"
                           class="btn-premium word-btn js-word-download"
                           data-download-word-url="{{ route('seat.export.word', $r->id) }}"
                           data-download-parts-url="{{ route('seat.export.parts_labour', $r->id) }}"
                           data-component="{{ $r->component ?? 'CCR Seat' }}">
                            Download
                        </a>
            ...
            @empty
                <p class="empty-state">Belum ada data CCR Seat.</p>
            @endforelse

            <p class="empty-state empty-state-filtered" id="noResultSeat" style="display:none;">
        ...
        <div id="seat-draft-choice-modal" class="draft-modal" aria-hidden="true">
            <div class="draft-modal__backdrop" data-draft-close></div>
            <div class="draft-modal__panel" role="dialog" aria-modal="true" aria-labelledby="seat-draft-modal-title">
                <h3 id="seat-draft-modal-title" class="draft-modal__title">Draft Ditemukan</h3>
                <p class="draft-modal__desc" id="seat-draft-modal-desc"></p>
                <div class="draft-modal__list" id="seat-draft-modal-list"></div>
                <div class="draft-modal__empty" id="seat-draft-modal-empty" style="display:none;">
                    Belum ada draft.
                </div>

                <div class="draft-modal__actions">
                    <button type="button" id="seat-draft-new-btn" class="draft-btn draft-btn--primary">Buat Baru</button>
                    <button type="button" id="seat-draft-cancel-btn" class="draft-btn draft-btn--ghost">Batal</button>
                </div>
            </div>
        </div>
        ...
        {{-- ======================= MODAL DOWNLOAD LIST ======================= --}}
        <div id="seatDownloadModal" class="export-modal" aria-hidden="true">
            <div class="export-modal-backdrop" data-export-close></div>
            <div class="export-modal-card" role="dialog" aria-modal="true" aria-labelledby="seatDownloadModalTitle">
                <h4 id="seatDownloadModalTitle" class="export-modal-title">Download File</h4>
                <p id="seatDownloadModalText" class="export-modal-text">
                    Pilih file yang ingin diunduh.
                </p>
                <div class="export-modal-list">
                    <a href="#" class="export-list-item export-list-item-word" id="seatDownloadWordLink">
                        <img src="/icons/word.svg" class="export-list-icon-word" alt="Word">
                        <span>Download Word CCR</span>
                    </a>
                    <a href="#" class="export-list-item export-list-item-excel" id="seatDownloadPartsLink">
                        <img src="/icons/excel.svg" class="export-list-icon-excel" alt="Excel">
                        <span>Download Excel Parts & Labour + Detail</span>
                    </a>
                </div>
                <div class="export-modal-actions">
                    <button type="button" class="export-btn export-btn-ghost" id="seatDownloadCancel">Batal</button>
                </div>
            </div>
        </div>
        ...
        <script>
            (() => {
                ...
                const modal = page.querySelector('#seatDownloadModal');
                const textEl = page.querySelector('#seatDownloadModalText');
                const wordEl = page.querySelector('#seatDownloadWordLink');
                const partsEl = page.querySelector('#seatDownloadPartsLink');
                const cancelEl = page.querySelector('#seatDownloadCancel');
                ...
                function openDownloadModal(link) {
                    ...
                    const component = String(link?.dataset?.component || 'CCR Seat').trim();
        ...
        <script>
            (() => {
                ...
                const modal = page.querySelector('#seat-draft-choice-modal');
                if (!modal) return;

                const titleEl = page.querySelector('#seat-draft-modal-title');
                const descEl = page.querySelector('#seat-draft-modal-desc');
                const listEl = page.querySelector('#seat-draft-modal-list');
                const emptyEl = page.querySelector('#seat-draft-modal-empty');
                const btnNew = page.querySelector('#seat-draft-new-btn');
                const btnCancel = page.querySelector('#seat-draft-cancel-btn');
                ...
                async function fetchServerDrafts(kind) {
                    const type = kind === 'seat' ? 'seat' : 'seat';
                    ...
                    try {
                        ...
                        return rows.map((row) => {
                            ...
                            return {
                                ...
                                source: 'server',
                                kind,
                                name: String(row.draft_name || '').trim() || 'SEAT Draft',
                ...
                if (typeof clearCreateDraft === 'string' && clearCreateDraft === 'seat') {
                    clearDraft('seat', { server: true }).catch(() => {});
                }
        ...
        <script>
            document.addEventListener("DOMContentLoaded", () => {
                ...
                const statusFilter   = page.querySelector("#statusFilterSeat");
                const list           = page.querySelector("#reportListSeat");
                const resultCountEl  = page.querySelector("#resultCountSeat");
                const noResultEl     = page.querySelector("#noResultSeat");
    ...
    [data-page="manage-seat"]{
        --seat-accent: #E40505;
        --seat-red: #E11D48;
        --seat-red-soft: rgba(225,29,72,.14);
        --seat-surface: #ffffff;
        --seat-muted-surface: #f6f9ff;
        --seat-border: #d7e1ef;
        --seat-text: #0f172a;
        --seat-muted: #5f6d84;
        --seat-shadow: 0 3px 10px rgba(0,0,0,.07);
    ...
    [data-page="manage-seat"] .top-toolbar{
        display:flex;
        align-items:center;
    ...
    [data-page="manage-seat"] .btn-back:hover{
        background:#2f3439;
    }
    [data-page="manage-seat"] .top-toolbar-actions{
        display:flex;
        align-items:center;
        gap:10px;
        flex-wrap:wrap;
    }
    [data-page="manage-seat"] .btn-top{
        display:inline-flex;
        align-items:center;
    ...
    [data-page="manage-seat"] .btn-top-primary{
        color:#fff;
        background:#0D6EFD;
        box-shadow:0 6px 14px rgba(13,110,253,.24);
    }
    [data-page="manage-seat"] .btn-top-primary:hover{
        background:#0b5ed7;
    }
    ...
    [data-page="manage-seat"] .header-left{
        display:flex;
        align-items:center;
        gap:18px;
    }
    [data-page="manage-seat"] .header-logo{
        width:80px;
        height:80px;
        object-fit:contain;
    }
    [data-page="manage-seat"] .header-title{
        margin:0;
        font-size:20px;
        line-height:1.1;
        font-weight:800;
    ...
    [data-page="manage-seat"] .header-subtitle{
        margin:4px 0 0;
    ...
    [data-page="manage-seat"] .stats-grid{
        display:grid;
        grid-template-columns:repeat(5, minmax(130px, 1fr));
        gap:10px;
        margin-bottom:16px;
    }
    [data-page="manage-seat"] .stat-card{
        background:#fff;
        border:1px solid var(--seat-border);
        border-radius:10px;
        box-shadow:var(--seat-shadow);
    ...
    [data-page="manage-seat"] .stat-label{
        color:var(--seat-muted);
        font-weight:700;
        font-size:11px;
    ...
    [data-page="manage-seat"] .stat-value{
        margin-top:2px;
        color:var(--seat-text);
        font-weight:800;
        font-size:20px;
    ...
    [data-page="manage-seat"] .filter-box{
        background:#ffffff;
    }
    [data-page="manage-seat"] .section-title{
        margin:0 0 16px;
        font-size:18px;
        font-weight:700;
        line-height:1.15;
    ...
    [data-page="manage-seat"] .filter-group{
        min-width:0;
        display:flex;
        flex-direction:column;
        gap:8px;
    }
    [data-page="manage-seat"] .filter-group.filter-large{
        padding-right:0;
    }
    [data-page="manage-seat"] .filter-group.filter-small{
        padding-left:0;
    }
    [data-page="manage-seat"] .filter-group label{
        display:block;
        margin:0;
        color:#1e293b;
        font-weight:700;
    ...
    [data-page="manage-seat"] .input:focus{
        outline:none;
        border-color:var(--seat-accent);
        box-shadow:0 0 0 4px rgba(228,5,5,.12);
    }

    [data-page="manage-seat"] .report-list-box{
        padding-top:18px;
    }
    [data-page="manage-seat"] .list-pagination{
        margin-top:12px;
    }
    [data-page="manage-seat"] .list-head{
        display:flex;
        align-items:center;
    ...
    [data-page="manage-seat"] .list-head-tools{
        display:inline-flex;
        align-items:center;
        gap:10px;
        flex-wrap:wrap;
    }
    [data-page="manage-seat"] .btn-list-create{
        display:inline-flex;
        align-items:center;
        justify-content:center;
        width:auto;
    ...
    [data-page="manage-seat"] .btn-list-create:hover{
        background:#f8fafc;
        color:#111;
    ...
    [data-page="manage-seat"] .btn-list-create:active{
        transform:translateY(0) scale(1);
        filter:none;
        box-shadow:0 2px 5px rgba(228,5,5,.12);
    }
    [data-page="manage-seat"] .btn-list-create:focus-visible{
        outline:none;
        border-color:#E40505;
        box-shadow:
    ...
    [data-page="manage-seat"] .list-head-title{
        font-size:14px;
        color:#71809b;
        font-weight:800;
        letter-spacing:.08em;
    ...
    [data-page="manage-seat"] .list-head-count{
        display:inline-flex;
        align-items:center;
        gap:6px;
        padding:6px 11px;
    ...
    [data-page="manage-seat"] .list-head-count .count-number{
        font-size:15px;
        line-height:1;
        letter-spacing:-.02em;
    }
    [data-page="manage-seat"] .list-head-count .count-text{
        font-size:12px;
        font-weight:700;
    }
    ...
    [data-page="manage-seat"] .select-all-label b{
        font-weight:800;
    }
    [data-page="manage-seat"] .select-all-hint{
        color:var(--seat-muted);
        font-size:13px;
        font-weight:600;
    }
    ...
    [data-page="manage-seat"] .report-card:hover{
        border-color:#cfdbec;
    ...
    [data-page="manage-seat"] .status-text{
        line-height:1;
    }
    [data-page="manage-seat"] .status-pill-draft{
        color:#64748b;
        background:rgba(100,116,139,.10);
        border-color:rgba(100,116,139,.35);
    }
    [data-page="manage-seat"] .status-pill-waiting{
        color:#de9b2d;
        background:rgba(222,155,45,.14);
        border-color:rgba(245,207,72,.78);
    }
    [data-page="manage-seat"] .status-pill-approved{
        color:#43c06a;
        background:rgba(67,192,106,.14);
        border-color:rgba(129,236,168,.72);
    }
    [data-page="manage-seat"] .status-pill-rejected{
        color:#ff6d5a;
    ...
    [data-page="manage-seat"] .status-note-btn:hover{ filter:brightness(.98); }
    ...
    [data-page="manage-seat"] .time-wrap{
        display:flex;
        gap:8px;
        flex-wrap:wrap;
    }
    [data-page="manage-seat"] .time-pill{
        background:#fff1f2;
    ...
    [data-page="manage-seat"] .time-wita{
        font-weight:900;
        color:#dc2626;
    }
    [data-page="manage-seat"] .time-wita{
    ...
    [data-page="manage-seat"] .btn-premium:hover{
        text-decoration:none;
        transform:translateY(-1px);
        box-shadow:0 4px 10px rgba(0,0,0,.15);
    }
    [data-page="manage-seat"] .lihat-btn{ background:#f57c00; }
    [data-page="manage-seat"] .lihat-btn:hover{ background:#e57200; }
    [data-page="manage-seat"] .edit-btn,
    [data-page="manage-seat"] .word-btn{
        background:#f8fafc;
        color:#111827;
        border:2px solid #c4ccd8;
        box-shadow:none;
    }
    [data-page="manage-seat"] .edit-btn:hover,
    [data-page="manage-seat"] .word-btn:hover{
        background:#f1f5f9;
        color:#0f172a;
        border-color:#b8c2cf;
        transform:none;
    ...
    [data-page="manage-seat"] .btn-submit{ background:#9f8170; }
    [data-page="manage-seat"] .btn-submit:hover{ background:#8f7465; }
    [data-page="manage-seat"] .btn-submit,
    ...
    [data-page="manage-seat"] .btn-inreview:hover{
        background:#d7dce5 !important;
        color:#586478 !important;
        box-shadow:none !important;
        transform:none !important;
    ...
    [data-page="manage-seat"] .btn-inreview:focus,
    [data-page="manage-seat"] .btn-inreview:focus-visible,
    [data-page="manage-seat"] .btn-inreview:active{
        outline:none !important;
        box-shadow:none !important;
        transform:none !important;
    }
    [data-page="manage-seat"] .icon-btn{
        width:16px;
        height:16px;
    }

    [data-page="manage-seat"] .empty-state{
        margin:14px 4px 2px;
        padding:22px;
    ...
    [data-page="manage-seat"] .draft-modal{
        position:fixed;
        inset:0;
        z-index:1200;
        display:none;
    }
    [data-page="manage-seat"] .draft-modal.is-open{
        display:block;
    }
    [data-page="manage-seat"] .draft-modal__backdrop{
        position:absolute;
        inset:0;
        background:rgba(2,6,23,.58);
    }
    [data-page="manage-seat"] .draft-modal__panel{
        position:relative;
    ...
    [data-page="manage-seat"] .draft-modal__title{
        margin:0;
        font-size:22px;
        font-weight:900;
        color:#0f172a;
    }
    [data-page="manage-seat"] .draft-modal__desc{
        margin:8px 0 0;
        font-size:14px;
        line-height:1.45;
        color:#475569;
    }
    [data-page="manage-seat"] .draft-modal__list{
        margin-top:12px;
        max-height:320px;
        overflow:auto;
        display:grid;
    ...
    [data-page="manage-seat"] .draft-modal__empty{
        margin-top:12px;
    ...
    [data-page="manage-seat"] .draft-row{
        border:1px solid #dbe2ea;
        background:#fff;
        border-radius:12px;
        padding:8px;
    ...
    [data-page="manage-seat"] .draft-row__open{
        flex:1;
        border:1px solid #dbe2ea;
        border-radius:10px;
        background:#f8fafc;
    ...
    [data-page="manage-seat"] .draft-row__open:hover{
        border-color:#93c5fd;
        background:#eff6ff;
    }
    [data-page="manage-seat"] .draft-row__name{
        font-size:14px;
        font-weight:900;
        color:#0f172a;
    }
    [data-page="manage-seat"] .draft-row__meta{
        margin-top:4px;
        font-size:12px;
        font-weight:700;
        color:#475569;
    }
    [data-page="manage-seat"] .draft-row__delete{
    ...
    [data-page="manage-seat"] .draft-row__delete:hover{
        background:#ffe4e6;
    }
    [data-page="manage-seat"] .draft-modal__actions{
        margin-top:14px;
        display:flex;
        justify-content:flex-end;
        gap:10px;
    ...
    [data-page="manage-seat"] .draft-btn{
        border:1px solid #dbe2ea;
        border-radius:10px;
        padding:9px 14px;
        font-size:13px;
    ...
    [data-page="manage-seat"] .draft-btn:disabled{
        opacity:.65;
        cursor:not-allowed;
    }
    [data-page="manage-seat"] .draft-btn--primary{
        color:#fff;
        background:#2563eb;
        border-color:#2563eb;
    }
    [data-page="manage-seat"] .draft-btn--primary:hover{
        background:#1d4ed8;
    }
    [data-page="manage-seat"] .draft-btn--ghost{
        color:#1f2937;
        background:#fff;
    }
    [data-page="manage-seat"] .draft-btn--ghost:hover{
        background:#f8fafc;
    }
    ...
    [data-page="manage-seat"] .note-modal.is-open{
        display:block;
    }
    ...
    [data-page="manage-seat"] .export-modal{
        position:fixed;
        inset:0;
        display:none;
        z-index:9999;
    }
    [data-page="manage-seat"] .export-modal.is-open{
        display:block;
    }
    [data-page="manage-seat"] .export-modal-backdrop{
        position:absolute;
        inset:0;
        background:rgba(0,0,0,.20);
        backdrop-filter:blur(2px);
    ...
    [data-page="manage-seat"] .export-modal-card{
        position:relative;
    ...
    [data-page="manage-seat"] .export-modal-title{
        margin:0;
        font-size:22px;
        font-weight:900;
        color:#0f172a;
    }
    [data-page="manage-seat"] .export-modal-text{
        margin:10px 0 0;
        font-size:14px;
        line-height:1.5;
        color:#475569;
    ...
    [data-page="manage-seat"] .export-modal-list{
        margin-top:14px;
        display:flex;
        flex-direction:column;
        gap:10px;
    }
    [data-page="manage-seat"] .export-list-item{
        display:flex;
        align-items:center;
        gap:10px;
        width:100%;
    ...
    [data-page="manage-seat"] .export-list-item:hover{
        background:#eef5ff;
        border-color:#c4d6f5;
        color:#0b244a;
        text-decoration:none;
    }
    [data-page="manage-seat"] .export-list-item.is-disabled{
        opacity:.55;
        pointer-events:none;
    }
    [data-page="manage-seat"] .export-list-item.is-busy{
        opacity:.6;
        pointer-events:none;
        filter:saturate(.75);
    }
    [data-page="manage-seat"] .export-list-icon-word{
        width:18px;
        height:18px;
        object-fit:contain;
        flex:0 0 18px;
    }
    [data-page="manage-seat"] .export-list-icon-excel{
        width:18px;
        height:18px;
        object-fit:contain;
        flex:0 0 18px;
    }
    [data-page="manage-seat"] .export-list-item > span:last-child{
        min-width:0;
        overflow-wrap:anywhere;
        word-break:break-word;
    }
    [data-page="manage-seat"] .export-modal-actions{
        margin-top:14px;
        display:flex;
        justify-content:flex-end;
        gap:10px;
    ...
    [data-page="manage-seat"] .export-btn{
        border:1px solid #dbe2ea;
        border-radius:10px;
        padding:9px 14px;
        font-size:13px;
    ...
    [data-page="manage-seat"] .export-btn:disabled{
        opacity:.65;
        cursor:not-allowed;
    }
    [data-page="manage-seat"] .export-btn-ghost{
        background:#fff;
        color:#1f2937;
    }
    [data-page="manage-seat"] .export-btn-ghost:hover{
        background:#f8fafc;
    }
    [data-page="manage-seat"] .export-btn-primary{
        background:#185abd;
        border-color:#185abd;
        color:#fff;
    }
    [data-page="manage-seat"] .export-btn-primary:hover{
        background:#154ea4;
    }
