...
<div class="trash-page" data-page="trash-seat">
  ...
  {{-- ======================= FILTER BOX (LIVE) ======================= --}}
  <div class="box">
    <h3 style="margin-bottom:18px;">Daftar CCR Seat (Trash)</h3>
  ...
  >
    ...
    <div class="box" id="trashList" x-ref="list">
      ...
      <div class="select-all-row">
        ...
        >
        <label for="selectAllTrashSeat" class="select-all-label">Select All</label>
      </div>

      <div class="divider"></div>

      {{-- Empty message (akan di-hide/show oleh JS) --}}
      <p id="trashEmpty" class="empty-text" style="display:none;">Trash Seat masih kosong.</p>
      ...
      @foreach($reports as $r)
        <div class="report-card"
          ...
          <div class="report-actions">
            <form action="{{ route('trash.seat.restore', $r->id) }}" method="POST">
              @csrf
              <button class="btn-premium restore-btn" type="submit">♻️ Restore</button>
            </form>

            <form action="{{ route('trash.seat.force', $r->id) }}" method="POST"
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
      ...
      function applyFilters() {
        ...
        if (window.__trashSeatSyncSelectAll) window.__trashSeatSyncSelectAll();
      }
...
/* (STYLE SAMA PERSIS DENGAN ENGINE) */
.trash-page .btn-back{
...
.bulk-bar{
  ...
  padding:14px;border-radius:14px;background:white;
  box-shadow:0 3px 10px rgba(0,0,0,.07);
}
.bulk-form{ margin:0; }
.btn-bulk{
  display:inline-flex;align-items:center;gap:10px;padding:12px 18px;border-radius:12px;
  font-weight:800;border:0;cursor:pointer;color:white;
  box-shadow:0 3px 7px rgba(0,0,0,.15);transition:.2s;
}
...
.report-card.selected{
  background:#fff5f5;border:2px solid rgba(228,5,5,.25);
  box-shadow:0 0 14px rgba(228,5,5,.18);border-radius:14px
}
@media (max-width:600px){
